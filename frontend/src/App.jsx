  ...
  return <main className="relative flex min-h-[100svh] flex-col overflow-x-clip bg-[#c9c8c6] font-sans uppercase tracking-widest text-black">
    ...
    <div className="relative z-10 flex min-h-screen flex-col px-5 pt-5 sm:px-8 md:px-12 md:pt-6">
      <nav className="flex h-9 shrink-0 items-center justify-between" aria-label="Primary navigation">
        <motion.a custom={0} variants={fadeDown} initial={initial} animate="visible" href="/" aria-label="RetentionPulse home" className="flex items-center gap-2 rounded-full focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2"><Logo compact /><span className="text-xs font-semibold tracking-widest">RP</span></motion.a>
        <div className="hidden items-center gap-6 lg:flex">{navItems.map((item, index) => <motion.a key={item} custom={index + 1} variants={fadeDown} initial={initial} animate="visible" href={`#${item.toLowerCase()}`} className="text-sm font-semibold tracking-widest focus:outline-none focus:ring-2 focus:ring-accent">{item}</motion.a>)}</div>
...
function Dashboard() {
  ...
  const selectFile = (next) => { setError(''); setEnrichmentError(''); setAnalysis(null); if (!next) return; if (next.size > 250 * 1024 * 1024) { setFile(null); setError('This video exceeds the 250MB upload limit.'); return } setFile(next) }
  const readDuration = (next) => new Promise((resolve, reject) => { const url = URL.createObjectURL(next); const probe = document.createElement('video'); probe.preload = 'metadata'; probe.onloadedmetadata = () => { URL.revokeObjectURL(url); resolve(probe.duration) }; probe.onerror = () => { URL.revokeObjectURL(url); reject(new Error('This video could not be read by the browser.')) }; probe.src = url })
  const submit = async (event) => { event.preventDefault(); if (!file || busy || enriching) return; setBusy(true); setError(''); setEnrichmentError(''); try { const duration = await readDuration(file); if (!Number.isFinite(duration) || duration <= 0) throw new Error('This video has no readable duration.'); if (duration > 300) throw new Error('Videos must be five minutes or shorter.'); setAnalysis(await analyzeVideo(file, undefined, 'fast_preview')) } catch (err) { if (err.name !== 'AbortError') setError(err.message || 'The video could not be analyzed. Please try again.') } finally { setBusy(false) } }
  const enrich = async () => { if (!file || busy || enriching) return; setEnriching(true); setEnrichmentError(''); try { setAnalysis(await analyzeVideo(file, undefined, 'multimodal')) } catch (err) { if (err.name !== 'AbortError') setEnrichmentError(err.message) } finally { setEnriching(false) } }
