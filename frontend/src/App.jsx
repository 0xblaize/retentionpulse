...
function Dashboard() {
  ...
  const selectFile = (next) => { setError(''); setEnrichmentError(''); setAnalysis(null); if (!next) return; const valid = ['video/mp4', 'video/quicktime', 'video/x-m4v'].includes(next.type) || /\.(mp4|mov|m4v)$/i.test(next.name); if (!valid) { setFile(null); setError('Choose an MP4, MOV, or M4V video.'); return } if (next.size > 250 * 1024 * 1024) { setFile(null); setError('This video exceeds the 250MB upload limit.'); return } setFile(next) }
  const submit = async (event) => { event.preventDefault(); if (!file || busy || enriching) return; setBusy(true); setError(''); setEnrichmentError(''); try { setAnalysis(await analyzeVideo(file, undefined, 'fast_preview')) } catch (err) { if (err.name !== 'AbortError') setError(err.message) } finally { setBusy(false) } }
